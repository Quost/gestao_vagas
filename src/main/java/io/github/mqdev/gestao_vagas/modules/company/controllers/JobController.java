...
public class JobController {
    ...
    @GetMapping("/")
    @PreAuthorize("hasRole('COMPANY')")
    @Tag(name = "Vagas", description = "Listar vagas")
    @Operation(summary = "Listar as vagas", description = "Listar as vagas de uma empresa pelo id")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", content = {
                    @Content(schema = @Schema(implementation = JobEntity.class))
            }, description = "Vagas encontradas"),
    ...
    @SecurityRequirement(name = "jwt_auth")
    public ResponseEntity<Object> listJobsByCompany(HttpServletRequest request) {
        var companyId = request.getAttribute("company_id");
